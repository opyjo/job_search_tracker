...
const ResumePreview = ({
  ...
  const [editedExperiences, setEditedExperiences] = useState<Map<number, Experience>>(new Map());
  const [activeEditIndex, setActiveEditIndex] = useState<number | null>(null);
  const [loadingEditIndex, setLoadingEditIndex] = useState<number | null>(null);
  const [editInstruction, setEditInstruction] = useState<string>("");
  const [editError, setEditError] = useState<string | null>(null);

  const { tailored_resume: rawResume, optimization_notes } = data;
  ...
  const handleSubmitEdit = async (i: number) => {
    ...
    try {
      ...
      setEditedExperiences((prev) => {
        ...
        next.set(i, json.experience as Experience);
        return next;
      });
      setActiveEditIndex(null);
      setEditInstruction("");
          ...
          <section className="mb-8">
            ...
            <div className="space-y-6">
              {tailored_resume.experience.map((exp, index) => {
                const isEdited = editedExperiences.has(index);
                const isFormOpen = activeEditIndex === index;
                const isLoading = loadingEditIndex === index;
                return (
                    ...
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex items-center gap-1 ml-2">
                        <div className="opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                          <button
                            onClick={() => isFormOpen ? handleCancelEdit() : handleOpenEdit(index)}
                          ...
                          {isEdited && !isFormOpen && (
                            <button
                              onClick={() => handleRevertEdit(index)}
