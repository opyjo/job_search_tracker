...
const CoverLetterPreview = ({ data, companyName, onReset, onToast }: CoverLetterPreviewProps) => {
...
${cover_letter.signature.replace("\\n", "\n")}`;
  ...
  const handleDownloadDocx = async () => {
    ...
    try {
      const blob = await generateCoverLetterDocx(cover_letter, companyName);
      const fileName = `${candidateData.name.replace(/\s+/g, "_")}_CoverLetter_${companyName.replace(/\s+/g, "_")}_${new Date().toISOString().split("T")[0]}.docx`;
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      const blob = generateCoverLetterPdf(cover_letter, companyName);
      const fileName = `${candidateData.name.replace(/\s+/g, "_")}_CoverLetter_${companyName.replace(/\s+/g, "_")}_${new Date().toISOString().split("T")[0]}.pdf`;
      saveAs(blob, fileName);
    ...
    <div className="w-full">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-violet-200 text-sm">
                Tone: <span className="capitalize font-medium text-white">{metadata.tone_used}</span>
              </p>
            </div>
          </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-800">For {companyName}</h2>
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-slate-200 p-8 lg:p-12">
        ...
        <div className="mb-8 pb-6 border-b border-slate-200">
          <h3 className="text-xl font-bold text-slate-800">{candidateData.name}</h3>
          <p className="text-slate-600 text-sm">
            {candidateData.email} | {candidateData.phone} | {candidateData.location}
          </p>
          <p className="text-slate-600 text-sm">{candidateData.linkedin}</p>
        </div>
          ...
          <div className="text-slate-800">
            {cover_letter.signature.split("\\n").map((line, i) => (
              <p key={i} className={i === 0 ? "mb-1" : "font-semibold"}>
                {line.replace("[Name]", candidateData.name)}
              </p>
            ))}
          </div>
        </div>
      ...
      <div className="mt-6 bg-slate-50 rounded-2xl p-6">
        <h3 className="text-lg font-bold text-slate-800 mb-4">Generation Details</h3>
        
        <div className="grid md:grid-cols-2 gap-6">
          {/* Key Points Addressed */}
          <div>
            <h4 className="text-sm font-semibold text-slate-600 mb-2">Key Points Addressed</h4>
          ...
          <div>
            <h4 className="text-sm font-semibold text-slate-600 mb-2">Company-Specific Mentions</h4>
            <div className="flex flex-wrap gap-2">
