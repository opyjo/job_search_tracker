import { CandidateData, isDeveloperCandidate, isPayrollCandidate, isGRCCandidate, DynamicATSResume } from "./types";
...
// ============================================
// DYNAMIC ATS RESUME COVER LETTER PROMPT
// ============================================

export const generateCoverLetterSystemPromptFromDynamic = (resume: DynamicATSResume): string => {
  const skillsSummary = resume.skills
    .slice(0, 2)
    .map(group => group.items.join(", "))
    .join("; ");

  const keyAchievements = resume.experience[0]?.achievements.slice(0, 5) ?? [];
  const currentRole = resume.experience[0]?.role ?? resume.target_job_title;
...
**Target Role:** ${resume.target_job_title}
...
"When I saw that [Company] is building [specific product/feature], I immediately thought of my work at ${currentCompany || "my previous role"} where I delivered similar results. Your mission to [company mission] aligns perfectly with what I've been focused on as a ${resume.target_job_title}."

**Professional (for enterprises):**
"[Company]'s commitment to [specific value/initiative] resonates deeply with my experience delivering results at ${currentCompany || "my most recent role"}. I am excited to bring my expertise in [relevant skill] to your [specific team/project]."
